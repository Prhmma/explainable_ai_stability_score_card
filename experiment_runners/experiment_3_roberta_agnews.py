"""
Experiment 3: RoBERTa on AG-News Test Set

Evaluates stability of explanation methods for RoBERTa fine-tuned on AG-News
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('experiment_3.log'),
...
def main():
    """Run Experiment 3: RoBERTa on AG-News."""
    logger.info("Starting Experiment 3: RoBERTa on AG-News")
    
    # Experiment configuration
    model_name = EXPERIMENT_MODELS['experiment_3']['name']
    ...
    experiment_name = 'experiment_3_roberta_agnews'
    
    # Initialize assessor
    assessor = StabilityAssessor(
        results_dir='../results',
        random_seed=42,
        max_samples=None  # Use all 2000 stratified samples
    )
    ...
    try:
        ...
        print(f"Dataset: AG-News test stratified ({dataset_config['expected_size']} samples)")
        print(f"Classes: {dataset_config['expected_classes']} (World, Sports, Business, Sci/Tech)")
        print(f"Explainers: {len(explainer_names)}")
        print(f"Stress tests: {len(stress_test_names)}")
        print("Duration: %.1f seconds" % results['duration_seconds'])
        print("="*50)
        
    except Exception as e:
        logger.error("Experiment 3 failed: %s", str(e))
        raise
